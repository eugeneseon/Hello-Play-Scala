...
object HBaseJournalInit {
  ...
  def createTable(config: Config, table: String, family: String): Boolean = {
  ...
  def disableTable(config: Config, table: String): Unit = {
    val conf = getHBaseConfig(config)
    val admin = new HBaseAdmin(conf)
  ...
  def deleteTable(config: Config, table: String): Unit = {
    val conf = getHBaseConfig(config)
    val admin = new HBaseAdmin(conf)
  ...
  private def doInitTable(admin: HBaseAdmin, tableName: String, familyName: String): Boolean = {
    Logger.info("doInitTable......")
    
    val test = admin.tableExists(tableName)
    
    Logger.info(admin.tableExists(tableName).toString())
